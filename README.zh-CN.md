...
### 示例 1：多智能体头脑风暴工作流
创建一个工作流，将用户问题路由到 3 个专业智能体（产品经理、工程师、设计师），收集他们的响应，并将其综合为最终建议。

**节点：** Trigger → Dynamic Agent (PM) → Dynamic Agent (Engineer) → Dynamic Agent (Designer) → Merge → Agent (Synthesizer) → Output

### 示例 2：数据处理管道
从 API 获取数据，用自定义代码转换，用 LLM 分析，并存储结果。

**节点：** Trigger → HTTP (fetch data) → Code (transform) → Agent (analyze) → HTTP (save result) → Output

### 示例 3：定时报告生成
每天早上运行工作流以收集指标、生成洞察并发送摘要邮件。

**节点：** Trigger (scheduled) → HTTP (fetch metrics) → Agent (generate insights) → HTTP (send email) → Output
...
**API 密钥与模型提供商：**
1. 打开设置 → 模型
2. 添加你的提供商（OpenAI、Anthropic、Gemini 等）
3. 输入 API 密钥并配置模型
4. 为提供商设置默认模型
...
2. 运行 `ollama serve`
3. 在设置 → 模型中，添加自定义提供商，基础 URL 为 `http://localhost:11434`

**自定义 API 端点：**
在设置 → 自定义 API 中定义自定义 REST 端点，以在智能体或工作流节点中用作工具。
