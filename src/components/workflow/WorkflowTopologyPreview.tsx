...
function ChatGenericNode({ data, type }: { data: any; type: string }) {
  ...
  return (
    <div className="w-[240px] rounded-xl border bg-card text-card-foreground shadow-sm">
      <Handle type="target" position={Position.Left} className="w-3 h-3 border-2 bg-popover" style={{ borderColor: c.color }} />
      ...
      <Handle type="source" position={Position.Right} className="w-3 h-3 border-2 bg-popover" style={{ borderColor: c.color }} />
    </div>
  );
}
...
export function WorkflowTopologyPreview({ workflow }: { workflow: Workflow }) {
  ...
  return (
    <div className="relative flex-1 flex flex-col h-full">
      ...
      <div className="absolute top-3 left-3 z-10 bg-popover/80 backdrop-blur-sm border rounded-lg px-3 py-1 shadow-sm">
      ...
      <div className="absolute bottom-3 left-3 z-10 text-xs text-muted-foreground bg-popover/80 backdrop-blur-sm px-2 py-1 rounded border">
        {t('双击工作流进入会话')}
      </div>
    </div>
  );
