...
*   **Content:** App Logo, Main Nav (Chats, Workflows, Agents, Prompts), Bottom Nav (Theme Toggle, Settings).
*   **Visual:** Slightly muted background (`bg-muted/50`) or distinct border to separate from the workspace.
*   **Note:** Profile icon removed from bottom nav per v0.2 update.

### 2.2 Contextual Sidebar (Left-Middle)
*   **Width:** ~240px - 280px (Collapsible).
*   **Content:** Dynamic based on Global Nav selection:
    *   *Chat View:* Top tabs toggle between "Workflows" and "Sessions". Single-column layout.
    *   *Workflow View:* List of saved workflows.
    *   *Agents View:* List of configured agents.
    *   *Prompts/Settings/Profile:* No sidebar shown.
*   **Visual:** Border separated (`border-r`), clean list layout with hover states.
*   **List Item Design:** Avatar/Icon + Name + Status (ON badge) + Chevron indicator. Three-dot menu for edit/delete on hover.

### 2.3 Main Content Area (Right)
*   **Width:** Flexible (`flex-1`).
*   **Content:** The active view (Chat Interface, Workflow Canvas, Agent Settings, Settings).
...
*   **MiniMap:** Visual overview of the canvas. Positioned bottom-right. Supports dark/light theme colors.
...
*   **Models Tab:**
    ...
    *   **Right Panel (Provider Detail):**
        *   Header: Provider name + type label + Enable/Disable toggle + Delete button.
...
- [x] Initialize Vite/React project with `tailwindcss`, `lucide-react`, and `shadcn/ui`.
- [x] Setup base theme variables (globals.css) for Light/Dark mode.
- [x] Implement Application Shell (Global Sidebar + Context Sidebar + Main layout).
- [x] Build reusable UI primitives (Button, Input, Dialog, ScrollArea, Avatar, Tooltip).
- [x] Construct complex views (Chat View with mentions, Workflow Canvas).
- [x] Implement multi-provider model management UI (SettingsView).
- [x] Add MiniMap to Workflow Canvas.
- [ ] Implement responsive breakpoints and drawer mode for mobile.
